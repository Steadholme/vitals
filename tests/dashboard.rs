...
async fn empty_dashboard_renders_brand_and_empty_state() {
    ...
    assert!(html.contains("运行能力"), "runtime capability panel shown");
    assert!(html.contains("/api/metrics"), "metrics api link shown");
    assert!(html.contains("/api/anomalies"), "anomalies api link shown");
    assert!(html.contains("net_rx_bps"), "full metric vocabulary shown");
    assert!(html.contains("0 异常"), "calm anomaly state shown");
