...
#[test]
fn insert_is_idempotent_per_host_metric_ts() {
    let store = InMemoryStore::new();
    store.insert_samples("h1", &[s("cpu_pct", 10.0, 100)]);
    // Same (host, metric, ts) — first write wins (mirrors ON CONFLICT DO NOTHING).
    store.insert_samples("h1", &[s("cpu_pct", 99.0, 100)]);
    let rows = store.query(Some("h1"), Some("cpu_pct"), 0);
    assert_eq!(rows.len(), 1);
    assert_eq!(rows[0].value, 10.0);
}

#[test]
fn retention_prune_drops_old_samples() {
    let store = InMemoryStore::new();
    store.insert_samples(
        "h1",
    ...
    let removed = store.prune(2500);
    assert_eq!(removed, 2);
    let rows = store.query(None, None, 0);
    assert_eq!(rows.len(), 1);
    assert_eq!(rows[0].ts, 3000);
    // Pruning again removes nothing.
    assert_eq!(store.prune(2500), 0);
}
